...
def writeResultsToDatabase(detections, min_conf, start_datetime):
    connection = sl.connect('detections.db')
    with connection:
        connection.execute("""
        ...
        for detection in detections:
            for entry in detections[detection]:
                if entry[1] >= min_conf and (entry[0] in WHITE_LIST or len(WHITE_LIST) == 0):
                    bounds = detection.split(';')
                    names = entry[0].split('_')

                    connection.execute("""
                        INSERT INTO DETECTION (start, end, scientific_name, common_name, confidence)
                        values(?, ?, ?, ?, ?)
                    """, (
                        float(start_datetime) + float(bounds[0]),  # Start timestamp (epoch)
                        float(start_datetime) + float(bounds[1]),  # End datetime (epoch)
                        names[0], names[1],  # Scientific and localised names
                        float(entry[1])  # Confidence
                    ))

                    connection.commit()  # Write to database
                    row_count += 1
    print('DONE! WROTE', row_count, 'RESULTS.')
...
def main():
    ...
    start_datetime_from_filename = re.search('\d+', args.i)
    start_datetime = int(start_datetime_from_filename.group(0))
    writeResultsToDatabase(detections, min_conf, start_datetime)
